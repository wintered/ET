...
data<- read.csv("csv/throughput.csv.avg", sep = ',')
...
data <- na.omit(data)
plt <- ggplot(data, aes(y=throughput,x=solver,label=throughput,ymin=0)) 
...
plt <- plt + scale_y_continuous(
    labels = function(x) paste0(format(x, big.mark = ",", scientific = FALSE), " tests/s")

)
plt <- plt + labs(x = "solver", y = "throughput")
...
plt <- plt + theme(panel.grid.major = element_line(colour = alpha("gray", 0.1)),panel.grid.minor = element_blank())
plt <- plt + theme(strip.text.x = element_text(size = 11,face="bold"))
plt <- plt + theme(strip.text.y = element_text(size = 11,face="bold"))
ggsave("plots/throughput-plot.pdf", width=32, height=23, units = "cm")
